...
def connect_uzi():
    ...
    if not ports:
        raise RuntimeError("No COM ports found — UZI not connected.")
    uzi = serial.Serial(ports[-1])
    uzi.baudrate = 9600
    uzi.timeout = 5
    print(f"Connected to UZI at {ports[-1]}")
    return uzi
    ...
    for cyc in range(int(cycles)):
        for pulse in seq:
            ...
            for _ in range(n_on):
                ...
                iA = smu.measure_i(A)           # Id
                iB = smu.measure_i(B)           # Ig
                vA = smu.measure_v(A)           # Vd 
                vG = pulse                      # Vg
